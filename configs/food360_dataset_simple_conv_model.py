...
default_hooks = dict(
    ...
    checkpoint=dict(type='CheckpointHook', interval=4, save_best='accuracy', rule='greater'),
    sampler_seed=dict(type='DistSamplerSeedHook'),
    visualization=dict(type='CustomVisualizationHook', val_interval=10, test_interval=10))
...
vis_backends = [dict(type='LocalVisBackend'),
                dict(type='MLflowVisBackend',
                     ...
                     exp_name='food360',
                     run_name='food360_simple_conv_model',
                     tracking_uri=None
                     )]
...
model = dict(
    type='mmengine_custom.SimpleConvModel',
    num_classes=24,
...
train_dataloader = dict(
    batch_size=128,
    num_workers=4,
    dataset=dict(type='mmengine_custom.Fruits360Dataset',
                 meta_file='/hdd/side_projects/mmlab_tutorials/data/class_names_to_indices.json',
                 ann_file='/hdd/side_projects/mmlab_tutorials/data/train_list.json',
...
val_dataloader = dict(
    ...
    dataset=dict(type='mmengine_custom.Fruits360Dataset',
                 meta_file='/hdd/side_projects/mmlab_tutorials/data/class_names_to_indices.json',
                 ann_file='/hdd/side_projects/mmlab_tutorials/data/val_list.json',
                 pipeline=[dict(type='LoadImageFromFile'),
                           dict(type='Resize', scale=(128, 128), keep_ratio=False),
                           dict(type='ToTensor', keys=['img']),
                           dict(type='mmengine_custom.CustomPackClsInputs')]),
